...
public class OduzimanjeBinarnihBrojevaMetodom2Komplementa {
	...
	public static void main(String[] args) {
		...
		int binar2Komplement[]=new int[8];
		
		binar1=pretvoriUBinarniBroj(broj1);
		binar2=pretvoriUBinarniBroj(broj2);
		
		binar2Komplement=dajKomplementBroja(binar2);
		
		razlika=saberiBinarneBrojeve(binar1,binar2Komplement);
		
		ispisiRezultat(broj1, broj2, razlika);
		
		}
	
	
	private static int[] dajKomplementBroja(int[]binar2) {

		int inverse[] = new int[8];
		int jedinica[] = new int[]{0,0,0,0,0,0,0,1};
		int komplement[] = new int[8];
		
		for(int i=0;i<binar2.length;i++){
			
			if(binar2[i]==0) inverse[i]=1;
			if(binar2[i]==1) inverse[i]=0;
		}
		...
		System.out.println();
		komplement=saberiBinarneBrojeve(inverse,jedinica);
		
		for(int i=0;i<komplement.length;i++){
			
			System.out.print(komplement[i] + " ");
		}
	...
	* Funkcija estetski lijepo ispisuje sve članove niza integera(članovi niza sadrže cifre binarnog oblika broja).
	...
	private static void ispisiRezultat(int broj1, int broj2, int[] zbir) {
			...
			System.out.printf("Razlika brojeva %d i %d u binarnom obliku: ",broj1,broj2);
